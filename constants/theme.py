'''

LibreDownloader themeing options


'''
class tab_panel:
    '''

    None on colours means default
    '''

    height = 60

    background = 0xe1007a
    linear_gradient = None

    compositing = False
    compositing_shadow = False
    compositing_shadow_extend = False

    spacing = 0

    class appmenu:
        '''
        -1 on height means expand
        -1 on width means equal than height
        '''
        stroke = 0
        stroke_width = 0

        shadow = 0
        shadow_width = 0

        axis = True
        color = None
        width = 28
        height = 28
        rotation = -0.03 # in pi rads

    class notification:
        font_size = 8
        padding_x = 5
        padding_y = 2
        bold = True

        stroke = None
        stroke_width = 0

        shadow = 0
        shadow_width = 0

        foreground = 0xFFFFFF
        background = 0x87d3cb

    class tab:
        '''
        -1 on height means expand
        -1 on width means shrink to text

        font_size < 0 means default's multiplier
        font_size == 0 means default
        font_size > 0 means point size

        None in colours means default
        '''

        transform = ""
        bold = False

        width = -1
        height = -1

        font_size = 18
        padding = 13

        foreground = 0xFFFFFF

        stroke = (0, 0, 0, 0.5)
        stroke_width = 0

        shadow = 0
        shadow_width = 0

        background = (225, 0, 122, 0) #e1007a
        linear_gradient = None
        radial_gradient = None

        border_width = 0
        border_radius = 0
        border_color = None

    class tab_active(tab):
        stroke_width = 0
        background = 0xcd0069
        linear_gradient = 0xa60055

    class tab_hover(tab):
        radial_gradient = (255, 255, 255, 0.25)

class toolbar:
    foreground = 0x666666
    background = 0xFFFFFF

    class searchbox:
        border_color = 0xECECEC
        border_top_color = 0xD2D2D2

        caption = "Search on active downloads"
        caption_color = 0xAAAAAA

        background = 0xFFFFFF
        foreground = 0x666666

class button:
    class button_normal:
        border_width = 1
        border_radius = 1
        border_color_1 = 0xBABABA
        border_color_2 = 0xAAAAAA

        background_color_1 = 0xCCCCCC
        background_color_2 = 0xBCBCBC

        font_size = -1
        bold = False

        foreground = 0x666666

        stroke = 0x0
        stroke_width = 0

        shadow = 0
        shadow_width = 0

        text_margin = 5
        icon_margin = 8

        selected_bitmap = False # Use selected bitmap by default

    class button_clicked(button_normal):
        background_color_1 = 0xBCBCBC
        background_color_2 = 0xCCCCCC

    class button_active(button_normal):
        border_color_1 = 0xcd0069
        border_color_2 = 0xcd0069

        background_color_1 = 0xa60055
        background_color_2 = 0xcd0069

        selected_bitmap = True

    class button_active_clicked(button_active):
        background_color_1 = 0xcd0069
        background_color_2 = 0xa60055

class statusbar:
    compositing = False
    bold = True

    stroke = (0, 0, 0, 0.5)
    stroke_width = 0

    shadow = 0
    shadow_width = 0

    padding = 4

    font_size = -1
    foreground = 0xFFFFFF
    background = 0x757575

class statusbar_button(button):
    class button_normal(button.button_normal):
        border_radius = 0

    class button_clicked(button.button_clicked):
        border_radius = 0

    class button_active(button.button_active):
        border_radius = 0

    class button_active_clicked(button.button_active_clicked):
        border_radius = 0

class done_button(button):
    class button_normal(button.button_normal):
        border_color_1 = 0x61AB5A
        border_color_2 = 0x61AB5A

        background_color_1 = 0x74BB52
        background_color_2 = 0x589938

        foreground = 0xFFFFFF

    class button_clicked(button_normal):
        background_color_1 = 0x589938
        background_color_2 = 0x74BB52

class downloads:
    background = 0xEAEAEA

    class progress:
        wide = -1
        radius = 3
        background = 0x6E6E6E
        foreground = 0xe1007a

    class panel:
        background = 0xEAEAEA
        foreground = 0x333333

        class separator:
            background_color_1 = (204, 204, 204, 0.7)
            background_color_2 = (245, 245, 245, 0.5)

    class panel_alternative(panel):
        background = 0xDEDEDE

    class panel_active(panel):
        background = 0xb1e1dc

    class panel_hover(panel):
        pass

class player:
    background = 0x6E6E6E
    foreground = 0xFFFFFF

    class title:
        foreground = 0xFFFFFF

    class progress:
        wide = 11
        radius = 2
        background = 0xC7C7C7
        foreground = 0xe1007a

    class volume(progress):
        wide = 5


class browser:
    loading_css_url = "data:image/gif;base64,"
